...
- **Symlinks without team OS verification.** Symlinks are the default deployment-pointer mechanism, but degrade silently on Windows checkouts that lack both Developer Mode (OS-level symlink permission) and `core.symlinks=true` in git config (controls whether git materializes symlinks at checkout time, set with `git config core.symlinks true`). Verify the team's OS mix before defaulting; fall back to wrapper files when either prerequisite cannot be guaranteed. See [anti-patterns.md#symlinks-without-team-platform-verification](references/anti-patterns.md#symlinks-without-team-platform-verification).
