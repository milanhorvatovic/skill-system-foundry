...
**Agent-delegated evaluation.** For a higher-fidelity, semantic check, the host agent does the classifying and the script scores it — no API key, no provider coupling, no network. The loop has three steps:

1. `--emit-tasks <path>` walks the corpora and writes a task envelope — one task per prompt, each holding an opaque `id`, the `prompt`, and the candidate `name + description` cards — plus a self-contained `instructions` field. The expected target and the positive/negative label are withheld from the task so the prompt must actually be classified, not read off the payload.
2. The host agent reads the task file and writes a predictions file mapping every task `id` to the single best card `name`, or `null` when none fits — shape `{id: name | null}`.
