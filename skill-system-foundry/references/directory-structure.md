...
Cross-file references inside a skill follow standard markdown semantics: every link resolves relative to the file that contains it. Two scopes own their own subgraph — the skill root (containing `SKILL.md`, shared `references/`, `assets/`, `scripts/`) and each capability root (`capabilities/<name>/`). A capability reaches the shared skill root via the explicit `../../<dir>/<file>` form. The full rule, the liftability invariant, and the migration cheat sheet live in [path-resolution.md](path-resolution.md).
