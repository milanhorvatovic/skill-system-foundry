...
- **Reserved Windows base names:** Skill names whose stem (case-insensitive) matches one of `CON`, `PRN`, `AUX`, `NUL`, `COM1`–`COM9`, or `LPT1`–`LPT9` FAIL the validator. These names cannot exist on NTFS regardless of extension or case, so a skill named `con` would scaffold on POSIX and break on Windows. (`COM0` and `LPT0` are intentionally absent — Windows does not reserve the zero-suffixed forms.) The list lives at `skill.name.windows_reserved_names` in `configuration.yaml`.
...
- **Dangling and degraded symlinks surface as dedicated findings:** A symlink that exists in the working copy but whose target does not (the typical Windows-without-Developer-Mode failure mode) produces a WARN that names the link and explains the failure mode, rather than the generic "does not exist" message that hides the symlink semantics. The degraded form — where git materialised the would-be link as a small text file containing the relative target instead of a real symlink — is recognised by file shape and an unresolvable target. When the captured target *does* exist (e.g. a `CLAUDE.md` shim whose body is `AGENTS.md` and `AGENTS.md` is a sibling), the validator surfaces an INFO instead: that shape is byte-identical to a deliberate one-line content reference, so the validator cannot decide on its own — the INFO invites the author to verify intent on a fresh checkout.
- **Long-path budget:** Every bundled file's *archive path* — the skill basename joined with the file's path relative to the skill root, matching what `create_bundle` writes into the zip — is checked against `bundle.long_path.threshold` minus `bundle.long_path.user_prefix_budget` in `configuration.yaml`. The basename is part of the measurement because it is part of the path the integrator's filesystem actually receives at extract time; a path that exactly fits the budget at the skill root would otherwise overflow MAX_PATH once `<skill-name>/` is prepended. Skills surface a WARN at validate time and a FAIL at bundle time, with a post-assembly re-check on the assembled bundle so externally-included references and inlined orchestrated skills are covered too. Default threshold and prefix budget model a representative `C:\Users\<name>\<repo>\` install location; integrators can tune the YAML for unusually long paths.
- **Forward slashes in JSON output and findings:** Program-derived paths in `--json` payloads, FAIL/WARN/INFO messages, and human-mode diagnostic lines (e.g. `Bundling: …`, `✓ Bundle created: …`) are normalised to forward slashes via `to_posix()` regardless of host. Two deliberate exceptions stay native: user-input echoes that round-trip the user's typed argument verbatim (e.g. `Error: '<typed-path>' is not a directory`, `Validating: <typed-path>`), and internal data structures whose `file` field is opened directly by the rewriter (since POSIX permits `\` as a legal filename character, normalising could silently target the wrong file). The companion `file_rel` field on those rows carries the POSIX form for JSON consumers.
