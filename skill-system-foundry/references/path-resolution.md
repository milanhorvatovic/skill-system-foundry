...
A skill conforms when `broken_under_standard_semantics == 0`, `files_unreachable_from_root == 0`, and `unrouted_capabilities == []`. The `unrouted_capabilities` list names every capability whose `capability.md` path is not listed in `SKILL.md`'s router-shaped markdown table — the structured surface the agent harness reads to dispatch. No link-graph walk is involved: a capability that `SKILL.md` mentions only in prose (or in inline-code, or as a doc link to an asset) is unrouted, because the harness has no way to know it should reach that capability. `connected_components` stays in the report as a diagnostic — useful for distinguishing orphan clusters from incomplete-router cases when investigating a failure — but the gate proper reads the three signals above.
