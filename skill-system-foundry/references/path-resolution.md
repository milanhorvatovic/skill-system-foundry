...
`bundle.py` reuses this same definition of a reference (the configured `reference_patterns`) so the bundler and the validator agree on what a link is, and it mirrors the out-of-skill posture above: a reference that escapes the skill root to a non-existent resource is a bundle WARN, not a FAIL. See the bundling capability's "What Counts as a Reference" section for the bundle-side details.
