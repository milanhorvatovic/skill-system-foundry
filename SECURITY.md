...
- **SSRF** in `.github/scripts/tool-catalog-drift.py` `fetch()`: the upstream URL ultimately derives from an operator-pointable config file. `fetch()` validates the scheme (`https`) and host against a **hardcoded** allowlist (`ALLOWED_FETCH_HOSTS`, independent of the config) before the request. Redirects are routed through a custom `HTTPRedirectHandler` (`_AllowlistRedirectHandler`) that rejects any 30x whose target is off the allowlist **before** the follow-up request is issued, so a redirect cannot cause an outbound request to an off-allowlist host. The final landing URL is re-validated after the response as defense in depth.
