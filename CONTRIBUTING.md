...
Dependabot opens grouped pull requests weekly for the GitHub Actions and Python dev dependencies. Each ecosystem groups minor and patch bumps into one rolling PR; semver-major bumps split into standalone PRs. Every Dependabot PR is labeled automatically — `dependencies`, its ecosystem, and **`release: patch`** (the static label from `.github/dependabot.yaml`). A dependency bump is at most a project **patch**: dependencies are not shipped in the skill bundle (GitHub Actions are CI infra, `requirements-dev.txt` is dev-only), so a dependency's own semver-major is **not** the project's major and is never escalated to `release: major`. `dependabot-release-label.yaml` only backfills `release: patch` when a PR (e.g. a security-update group) inherits no `release:` label; a maintainer can still set a higher level by hand by **replacing** `release: patch` with `release: minor` / `release: major` when a bump genuinely warrants it (the `verify-pr-release-label` gate requires exactly one `release:` label, so swap it rather than adding a second). The auto-merge hold for semver-major and trust-boundary-action bumps (below) is driven by the Dependabot update-type metadata, not by this label.
