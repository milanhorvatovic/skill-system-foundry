...
The meta-skill ships its own corpus under `tests/skill-corpus/skill-system-foundry/` (`skill.json` plus one file per capability). Scoring is heuristic — pure stdlib, deterministic, and free (Jaccard token overlap against each unit's `name + description` card) — so it carries no API key or third-party dependency and runs on every PR via the `validate-examples` job in `python-tests.yaml` with `--soft` (advisory: threshold breaches don't fail the step, though FAIL-level findings like a malformed or stale corpus still do — and `validate-examples` is not a branch-protection required check, so it never blocks a merge regardless). Higher-fidelity activation testing against a real model is a separate, opt-in workstream and is deliberately not bundled, keeping the meta-skill stdlib-only and AI-agnostic. Both new files are held to the 90% per-file branch-coverage bar in `python-tests.yaml`. Evaluation settings (thresholds, stopwords) live under `skill.description.evaluation` in `scripts/lib/configuration.yaml`; the corpus format and unit card model are documented in `capabilities/validation/capability.md`.
