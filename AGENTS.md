...
└── .github/
    ...
    │   └── verify-pr-release-label.yaml ← required PR release-label gate
...
3. **Branch ruleset on `main`** includes a `required_status_checks` rule listing the checks that always fire on drift PRs. The wait-and-merge step has two distinct failure modes for this precondition: (a) if the rule is absent or empty, the workflow hard-fails immediately at the expected-count query with `::error::Could not determine the expected required-check count...` (the query reads the rule via `gh api repos/.../rules/branches/main`); (b) if the rule is present but the listed checks don't register on the PR (paths-filter mismatch on the consumer, App events not firing), the pre-watch polling guard times out after 90s with `::error::Only N of M required check runs appeared...`. Concretely the four distinct contexts that need to be listed are the two matrix entries from `.github/workflows/python-tests.yaml` — `test (ubuntu-latest, 3.12)` and `test (windows-latest, 3.12)` — the single job from `.github/workflows/verify-action-pins.yaml`, registered as the bare context `verify`, and the `verify-release-label` job from `.github/workflows/verify-pr-release-label.yaml`; all four carry no path filter, so they fire on every PR, which is the property a *global* required check needs (a required check that never fires on a given PR blocks that PR's merge indefinitely). GitHub registers each context as the bare job-name + matrix suffix — *without* a workflow-name prefix in this repo's setup — so the contexts in the ruleset are exactly `test (ubuntu-latest, 3.12)`, `test (windows-latest, 3.12)`, `verify`, and `verify-release-label`, not `python-tests / test (...)` or `Verify action pins / verify`. Distinct job names matter for branch-protection clarity: a duplicate context name across workflows would leave `gh pr checks --required` and the auto-merge wait-and-merge step unable to attribute a pass/fail back to the originating workflow (`release-prep.yaml` also defines a `test` job, but it runs only on `workflow_dispatch`, so it never registers as a PR check context and cannot collide). Two path-filtered checks are deliberately NOT listed, for the same fire-on-every-PR reason: `helper-tests (ubuntu-latest, 3.12)` / `helper-tests (windows-latest, 3.12)` from `.github/workflows/ci-helper-tests.yaml` (filtered to `.github/scripts/**`, `.github/tests/**`, the two workflow files, and `configuration.yaml`) and `shellcheck` (filtered to `.github/scripts/*.sh`). Both still run on the PRs that touch their paths — and a drift PR touches `configuration.yaml`, so `helper-tests` does fire on it — but only as non-required signals; the drift auto-merge's `gh pr checks --required --watch` waits on the four global contexts above (the rolling drift PR self-labels `release: patch` at creation, so `verify-release-label` passes on it). Listing a path-filtered check as required would hang every PR that does not touch its paths.
...
Releases are automated: dispatch the `release-prep.yaml` workflow with a target version (or with the input left empty to compute it from labels — see the next paragraph) and it bumps the three manifests, prepends the changelog, dry-builds the bundle, opens the release PR as oss-release-bot, has oss-automation-bot approve it, and auto-merges on green; `release-on-merge.yaml` then tags the merge commit and `release.yaml` builds the bundle and creates the GitHub Release with the zip + SHA256 attached at creation. The manual tooling below is what that workflow runs internally, and the path to use when dispatching is unavailable (offline, or regenerating a past release). Run full validation and tests before tagging by hand.

The `version` dispatch input is optional. Leave it empty and `release-prep` computes the next version from the `release:` labels of PRs merged since the last tag — highest bump wins, `release: skip` contributes nothing — via `scripts/compute_release_version.py` (run it standalone to preview: it prints the `X.Y.Z` to stdout and the per-PR reasoning to stderr; see its module docstring for the window algorithm and exit codes). Compute anchors the base to the canonical `SKILL.md` manifest and aborts if the manifest and the latest tag disagree. An unlabeled or multi-labeled in-window PR aborts with a per-PR fix hint — the required `verify-pr-release-label` gate makes this rare, but compute re-validates as the backstop; label the named PRs and re-dispatch. Passing an explicit version overrides compute for that one run but does not clear the label debt — the unlabeled PRs must still be labeled before the next computed release. The release PR opens self-labeled `release: skip`, so it never counts toward its own successor.
