...
Automated validation (`validate_skill.py`, `audit_skill_system.py`) handles many frontmatter, naming, line-count, and structural checks. Manual review still verifies markdown file-reference conventions (file-relative resolution per `references/path-resolution.md`, including the `../../<dir>/<file>` external-reference syntax used from capabilities), path validity, progressive disclosure, description quality, architecture justification, and semantic consistency.
