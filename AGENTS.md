...
1. **GitHub App installed on the repo** with permissions `contents: write` and `pull-requests: write`. The app id is stored as repository secret `DRIFT_BOT_APP_ID` and its PEM private key as `DRIFT_BOT_PRIVATE_KEY`. The App identity (not the default `GITHUB_TOKEN`) is required so the rolling PR's `push`/`pull_request` events trigger downstream workflows — GitHub deliberately excludes `GITHUB_TOKEN`-authored events from triggering further runs, which would otherwise leave the auto-merge wait with no checks to wait on.
