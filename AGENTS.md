...
3. **Branch ruleset on `main`** includes a `required_status_checks` rule listing the checks that always fire on drift PRs. The watch step's `--required` filter reads from this list; if the rule is absent or empty no required checks ever appear, the pre-watch polling guard in the wait-and-merge step times out after 90s with `::error::`, and the workflow fails without merging. Concretely the three contexts that need to be listed are `test (ubuntu-latest, 3.12)` and `test (windows-latest, 3.12)` (the two matrix entries from `.github/workflows/python-tests.yaml`) plus `test` (from `.github/workflows/ci-helper-tests.yaml`, which is paths-filtered to include `configuration.yaml` so it does fire on drift PRs). Both workflows name their job `test` and GitHub registers the check context as the bare job-name + matrix suffix — *without* a workflow-name prefix in this repo's setup — so the contexts in the ruleset are exactly those bare strings, not `python-tests / test (...)` or `Python tests / test (...)`. `shellcheck` is paths-filtered to `.github/scripts/*.sh` and does NOT fire on drift PRs, so it should NOT be listed.
