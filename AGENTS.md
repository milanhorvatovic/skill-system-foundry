...
Five repo-level preconditions must be in place for the auto-merge path to actually fire. If any is missing, the workflow still opens the rolling PR (the catalog-sync half is independent), but the merge half no-ops or fails until the precondition is restored. Recording them here because they are not derivable from the code and silently regress when a fork, repo-recreate, or org-policy reset clears them.

1. **GitHub App installed on the repo** with permissions `contents: write` and `pull-requests: write`. The app id is stored as repository variable `AUTOMATION_APP_ID` (App IDs are not sensitive — variable, not secret, so the value is visible in repo settings for debugging) and its PEM private key as repository secret `AUTOMATION_PRIVATE_KEY`. The App identity (not the default `GITHUB_TOKEN`) is required so the rolling PR's `push`/`pull_request` events trigger downstream workflows — GitHub deliberately excludes `GITHUB_TOKEN`-authored events from triggering further runs, which would otherwise leave the auto-merge wait with no checks to wait on.
2. **Repo setting "Allow GitHub Actions to create and approve pull requests"** enabled at *Settings → Actions → General → Workflow permissions*. Required for the legacy `GITHUB_TOKEN` PR-create path that runs the first time before the App is installed; once the App handles PR creation this is a defense-in-depth toggle, but it remains a hard prerequisite for the no-App fallback path.
3. **Repo setting "Allow auto-merge"** enabled at *Settings → General → Pull Requests*. `gh pr merge --auto` errors out when this is disabled and the workflow surfaces the failure with `::error::` — no silent stranding, but also no merge.
4. **Branch ruleset on `main`** includes a `required_status_checks` rule listing the checks that always fire on drift PRs. The watch step's `--required` filter reads from this list; if the rule is absent or empty the wait returns immediately with no checks waited on and the synchronous merge races CI. Concretely: the two `python-tests / test (...)` matrix entries (ubuntu-latest and windows-latest) plus `CI helper tests / test` (from `.github/workflows/ci-helper-tests.yaml`, which is paths-filtered to include `configuration.yaml` so it does fire on drift PRs). `shellcheck` is paths-filtered to `.github/scripts/*.sh` and does NOT fire on drift PRs, so it should NOT be listed.
