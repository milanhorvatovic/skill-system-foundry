...
3. **Branch ruleset on `main`** includes a `required_status_checks` rule listing the checks that always fire on drift PRs. The watch step's `--required` filter reads from this list; if the rule is absent or empty no required checks ever appear, the pre-watch polling guard in the wait-and-merge step times out after 90s with `::error::`, and the workflow fails without merging. Concretely: the two `python-tests / test (...)` matrix entries (ubuntu-latest and windows-latest) plus `CI helper tests / test` (from `.github/workflows/ci-helper-tests.yaml`, which is paths-filtered to include `configuration.yaml` so it does fire on drift PRs). `shellcheck` is paths-filtered to `.github/scripts/*.sh` and does NOT fire on drift PRs, so it should NOT be listed.
