...
- **Review policy files (prompt, reference, schema) are sourced from the PR head.** This is intentional: when a PR modifies these files, the review should exercise the updated methodology so the changes are validated in-context. CODEOWNERS requires code-owner approval for `.github/codex/` changes, preventing unauthorized modifications from reaching the review pipeline.
