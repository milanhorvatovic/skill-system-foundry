...
- **Publish job executes scripts from prepare artifacts (PR head checkout).** This is accepted because CODEOWNERS requires code-owner approval for `.github/scripts/` changes, the publish token can only post PR comments (not push code, merge, or access secrets), and adding `contents: read` to the publish job for a base-branch checkout would broaden its permission surface.
