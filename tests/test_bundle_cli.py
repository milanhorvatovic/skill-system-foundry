...
class TargetFlagTests(unittest.TestCase):
    ...
    def test_env_var_fallback_used_when_no_flag(self) -> None:
        """SKILL_BUNDLE_TARGET env var is used as default when --target is omitted."""
        with tempfile.TemporaryDirectory() as tmpdir:
            system_root = os.path.join(tmpdir, "system")
            skill_dir = os.path.join(system_root, "skills", "demo-skill")
            write_text(os.path.join(system_root, "manifest.yaml"), "name: demo\n")
            ...
            env = os.environ.copy()
            env["SKILL_BUNDLE_TARGET"] = "gemini"
    ...
    def test_cli_flag_overrides_env_var(self) -> None:
        """--target flag takes precedence over SKILL_BUNDLE_TARGET env var."""
        with tempfile.TemporaryDirectory() as tmpdir:
            system_root = os.path.join(tmpdir, "system")
            skill_dir = os.path.join(system_root, "skills", "demo-skill")
            write_text(os.path.join(system_root, "manifest.yaml"), "name: demo\n")
            ...
            env = os.environ.copy()
            env["SKILL_BUNDLE_TARGET"] = "gemini"  # would pass without --target
            ...
            # --target claude should override env var and fail
            self.assertEqual(proc.returncode, 1)
            self.assertIn("Description is 201 characters", proc.stdout)
    ...
    def test_invalid_env_var_fails_fast(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            env = os.environ.copy()
            env["SKILL_BUNDLE_TARGET"] = "cluade"  # typo
            ...
            self.assertEqual(proc.returncode, 1)
            self.assertIn("Invalid SKILL_BUNDLE_TARGET", proc.stdout)
